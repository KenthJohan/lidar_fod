...
static void ce30_detect_incidence_edges (uint8_t flags[CE30_WH])
{
	...
	for (int32_t v = 1; v < CE30_H-1; ++v)
	{
		...
		if ((flags[i11] & POINT_GOOD) == 0)
		{
			if (flags[i00] & POINT_GOOD) {flags[i00] |= POINT_EDGE;}
			if (flags[i01] & POINT_GOOD) {flags[i01] |= POINT_EDGE;}
			if (flags[i02] & POINT_GOOD) {flags[i02] |= POINT_EDGE;}
			if (flags[i20] & POINT_GOOD) {flags[i20] |= POINT_EDGE;}
			if (flags[i21] & POINT_GOOD) {flags[i21] |= POINT_EDGE;}
			if (flags[i22] & POINT_GOOD) {flags[i22] |= POINT_EDGE;}
			if (flags[i10] & POINT_GOOD) {flags[i10] |= POINT_EDGE;}
			if (flags[i12] & POINT_GOOD) {flags[i12] |= POINT_EDGE;}
		}
	}
}
